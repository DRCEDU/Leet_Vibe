...
A comprehensive converter class for transforming HTML webpage packages 
...
from .utils import (
    setup_logging, validate_html_file, sanitize_filename, 
...
class HTMLToMarkdownConverter:
    ...
    def __init__(self, html_file_path: str, output_file: Optional[str] = None, config: Optional[Config] = None):
        """
        ...
        if not is_valid:
            raise ValueError(f"Invalid HTML file: {error_msg}")
        ...
        if output_file:
            ...
            output_dir = ensure_directory(self.config.get('output_dir', 'output'))
            filename = sanitize_filename(f"{self.html_file_path.stem}.md")
            self.output_file = output_dir / filename
        
        # Create images directory
        self.images_dir = ensure_directory(
            self.output_file.parent / self.config.get('images_dir', 'images')
        )
...
Examples:
  ...
  python html_to_markdown_converter.py "/path/to/webpage.html" "/path/to/output.md"
    ...
    try:
        ...
        converter = HTMLToMarkdownConverter(args.html_file, args.output_file)
